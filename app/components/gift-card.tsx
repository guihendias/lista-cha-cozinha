...
export function GiftCard({ gift, onReserve, index }: GiftCardProps) {
  const isFullyReserved = gift.is_fully_reserved;

  return (
    <motion.div
      layout
      initial={{ opacity: 0, y: 30 }}
      animate={{ opacity: isFullyReserved ? 0.5 : 1, y: 0 }}
      exit={{ opacity: 0, y: -20 }}
      ...
      whileHover={isFullyReserved ? {} : { y: -4 }}
    >
        ...
        {isFullyReserved ? (
          <>
            <p className="mt-2 flex items-center gap-1 text-xs text-text-light">
              <span>✓</span> Reservado por {gift.reservation_names.join(", ")}
            </p>
